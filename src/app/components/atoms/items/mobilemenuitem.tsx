...
const SubItemSection = ({ subItem, onNavigate }: { subItem: any, onNavigate: () => void }) => (
  <div className="mt-2">
    <p className="text-gray-800 font-semibold">{subItem.name}</p>
    <div>
      {subItem.subSections?.map((subSection: any, subSecIndex: number) => (
        <SubItemLink key={subSecIndex} subSection={subSection} onNavigate={onNavigate} />
      ))}
    </div>
  </div>
);
      ...
      {isOpen && (
        <div className="pl-4">
          {item.subItems?.map((subItem, subIndex) => (
            <SubItemSection key={subIndex} subItem={subItem} onNavigate={onNavigate} />
          ))}
        </div>
      )}
    </div>
